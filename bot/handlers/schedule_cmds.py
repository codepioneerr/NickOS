"""
bot/handlers/schedule_cmds.py
Commands: /schedule, /nudge, /health
"""

from telegram import Update
from telegram.ext import ContextTypes
from telegram.constants import ParseMode
from datetime import datetime

from bot.utils import (
    ET, now_et, today_str, WORKOUT_DAYS, WORKOUT_PLAN,
    SLEEP_TARGET, WATER_GOAL,
    get_water_today, get_sleep_streak, get_meals_today,
    get_workout_this_week, get_weekly_stats,
)


# ── /schedule ─────────────────────────────────────────────────────────────────
# Full schedule of daily nudge times
DAILY_SCHEDULE = [
    ("08:00", "⏰ Wake up nudge"),
    ("09:00", "🌅 Breakfast reminder"),
    ("09:00", "💧 Water nudge #1"),
    ("11:00", "💧 Water nudge #2"),
    ("13:00", "💧 Water nudge #3"),
    ("14:00", "☀️ Lunch reminder"),
    ("15:00", "💧 Water nudge #4"),
    ("17:00", "💧 Water nudge #5"),
    ("18:00", "💪 Workout nudge (if workout day)"),
    ("19:00", "🌙 Dinner reminder + Water nudge #6"),
    ("21:00", "💧 Water nudge #7"),
    ("22:00", "💧 Water nudge #8"),
    ("23:30", "🌙 Bedtime nudge (target: midnight)"),
]

async def schedule_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    /schedule — show today's remaining scheduled nudge times
    """
    now     = now_et()
    weekday = now.strftime("%A")
    current_time = now.strftime("%H:%M")

    lines = []
    past_count = 0

    for time_str, label in DAILY_SCHEDULE:
        # Skip workout nudge on non-workout days
        if "Workout nudge" in label and weekday not in WORKOUT_DAYS:
            continue

        if time_str > current_time:
            lines.append(f"  {time_str} — {label}")
        else:
            past_count += 1

    workout_note = ""
    if weekday in WORKOUT_DAYS:
        plan = WORKOUT_PLAN.get(weekday, "")
        workout_note = f"\n\n<b>💪 Today's workout:</b> <i>{plan}</i>"

    if lines:
        remaining = "\n".join(lines)
        msg = (
            f"📅 <b>Today's Remaining Schedule</b>\n"
            f"<i>{weekday}, {today_str()} — {past_count} nudges already sent</i>\n"
            f"\n"
            f"{remaining}"
            f"{workout_note}"
        )
    else:
        msg = (
            f"✅ <b>All nudges sent for today.</b>\n"
            f"Last one: bedtime. You going to bed on time tonight?"
            f"{workout_note}"
        )

    await update.message.reply_text(msg, parse_mode=ParseMode.HTML)


# ── /nudge ────────────────────────────────────────────────────────────────────
async def nudge_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    /nudge — trigger an immediate status check + nudge from Claude
    """
    import os
    import anthropic
    from dotenv import load_dotenv
    load_dotenv()

    now     = now_et()
    weekday = now.strftime("%A")
    water   = get_water_today()
    meals   = get_meals_today()
    workouts = get_workout_this_week()
    sleep   = get_sleep_streak()

    prompt = (
        f"You are NickOS — Nick's personal AI operating system. Aggressive tough-love tone. Short. Direct.\n\n"
        f"Current status:\n"
        f"- Time: {now.strftime('%I:%M %p ET')}, {weekday}\n"
        f"- Water today: {water}/8 glasses\n"
        f"- Meals today: {meals}\n"
        f"- Workouts this week: {workouts}/5\n"
        f"- Sleep streak: {sleep} days\n"
        f"- Today is {'a workout day' if weekday in WORKOUT_DAYS else 'a rest day'}\n\n"
        f"Give Nick a 2-3 line status check nudge. Start with an emoji. "
        f"Call out whatever is most behind. Tough love, no softening. "
        f"End with one specific action he should do RIGHT NOW."
    )

    try:
        client = anthropic.Anthropic(api_key=os.environ["ANTHROPIC_API_KEY"])
        resp   = client.messages.create(
            model=os.environ.get("ANTHROPIC_HAIKU_MODEL", "claude-haiku-4-5-20251001"),
            max_tokens=150,
            temperature=0.7,
            messages=[{"role": "user", "content": prompt}]
        )
        nudge_text = resp.content[0].text.strip()
    except Exception as e:
        nudge_text = (
            f"⚡ <b>Status check:</b>\n"
            f"Water: {water}/8 | Meals: {meals} | Workouts: {workouts}/5\n"
            f"Pick the worst number and fix it right now."
        )

    await update.message.reply_text(nudge_text, parse_mode=ParseMode.HTML)


# ── /health ───────────────────────────────────────────────────────────────────
async def health_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    /health — show this week's health stats dashboard
    """
    stats   = get_weekly_stats()
    now     = now_et()
    weekday = now.strftime("%A")
    water_today = get_water_today()

    # Sleep rating
    sleep_avg = stats["sleep_avg"]
    if sleep_avg >= SLEEP_TARGET:
        sleep_icon = "✅"
    elif sleep_avg >= 6.5:
        sleep_icon = "😐"
    else:
        sleep_icon = "🔴"

    # Water rating
    water_avg = stats["water_avg"]
    if water_avg >= WATER_GOAL:
        water_icon = "✅"
    elif water_avg >= 5:
        water_icon = "😐"
    else:
        water_icon = "🔴"

    # Workout rating (5x/week target)
    workouts = stats["workouts"]
    if workouts >= 5:
        workout_icon = "✅"
    elif workouts >= 3:
        workout_icon = "😐"
    else:
        workout_icon = "🔴"

    # Meals rating (target: 21 meals in 7 days = 3/day)
    meals = stats["meals_total"]
    expected = stats["days_counted"] * 3
    if meals >= expected * 0.9:
        meal_icon = "✅"
    elif meals >= expected * 0.6:
        meal_icon = "😐"
    else:
        meal_icon = "🔴"

    # Overall health score (0-100)
    score = 0
    score += min(25, int((sleep_avg / SLEEP_TARGET) * 25))
    score += min(25, int((water_avg / WATER_GOAL) * 25))
    score += min(25, int((workouts / 5) * 25))
    score += min(25, int((meals / max(expected, 1)) * 25))

    if score >= 85:
        score_label = "🔥 Locked in"
    elif score >= 65:
        score_label = "👍 Decent"
    elif score >= 45:
        score_label = "⚠️ Slipping"
    else:
        score_label = "🚨 Fix this now"

    msg = (
        f"📊 <b>Weekly Health Report</b>\n"
        f"<i>{stats['days_counted']} days logged</i>\n"
        f"\n"
        f"{sleep_icon} <b>Sleep avg:</b> {sleep_avg}h (target: {SLEEP_TARGET}h)\n"
        f"{water_icon} <b>Water avg:</b> {water_avg}/day (today: {water_today}/8)\n"
        f"{meal_icon} <b>Meals:</b> {meals}/{expected} logged\n"
        f"{workout_icon} <b>Workouts:</b> {workouts}/5 this week\n"
        f"\n"
        f"<b>Health Score: {score}/100</b> — {score_label}"
    )

    # Attach today's workout if applicable
    if weekday in WORKOUT_DAYS:
        plan = WORKOUT_PLAN.get(weekday, "")
        msg += f"\n\n<b>Today:</b> <i>{plan}</i>"

    await update.message.reply_text(msg, parse_mode=ParseMode.HTML)
